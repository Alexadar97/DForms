...
@Component({
  selector: 'app-protobca',
  templateUrl: './protobca.component.html',
  styleUrls: ['./protobca.component.css']
})
export class ProtobcaComponent implements OnInit {
  ...
  isProtoEandE = false;
  isProtoPPS = false;
  isProtoMaintenance = false;
  isProtoAggregate = false;
  isProtoMechanical = false;
  fitmentreport;
  isfitment;
  aodrawing;
  aodrawingtext;
  ...
  constructor(notifierService: NotifierService, private _location: Location, private route: ActivatedRoute, private ds: DataserviceService, private fb: FormBuilder, private http: Http, private router: Router) {
    ...
    this.prmasterid = userJson['formid']
  ...
  addbca() {
    ...
    const bcaform = this.fb.group({
      ...
      objfinasid: [''],
      observation: [''],
      workstarteddate: ['' + this.todayDate],
      workendsdate: ['' + this.todayDate],
      actualkm: [''],
      workdoneby: [''],
      supervisorname: [''],
      supervisordate: ['' + this.todayDate],
      supervisorreceiveddate: ['' + this.todayDate],
      supervisorcompleteddate: ['' + this.todayDate],
      finasreqname: [''],
      finasreqdate: ['' + this.todayDate],
      finasreqcontactno: ['', num],
      finasreqreturndate: ['' + this.todayDate],
      isfitmentcreated:[''],
      finasreqreceiveddate: ['' + this.todayDate],
      finasreqremarks: [''],
      usertype: [],
      isDraft: [],
      islogin: [1],
  ...
  emptybca = false
  getbcasingledata() {
    var submitData = "prmasterid=" + this.prmasterid + "&usertype=" + this.status;
  ...
  fitmentposition;
  id: any;
  //   saveFitmentReport(value, i) {
  //     // this.router.navigate(['/retrofitmentnew']);
